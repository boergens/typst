...
fn test() {
    ...
    let (idle_sender, idle_receiver) = crossbeam_channel::unbounded();
    let (work_sender, work_receiver) = crossbeam_channel::unbounded();
    std::thread::scope(|scope| {
        let logger = &logger;

        // Add all tests to the channel.
        for test in tests.iter() {
            work_sender.send(test).unwrap();
        }
        ...
        let workers = (0..num_workers.get())
            .map(|_| {
                ...
                scope.spawn(move || {
                    ...
                    while let Ok(test) = work_receiver.try_recv() {
