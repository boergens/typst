    ...
    fn eval(self, vm: &mut Vm) -> SourceResult<Self::Output> {
        ...
        match &source {
            ...
            v if RootedPath::castable(v) => {
                let path = v.clone().cast::<RootedPath>().at(source_span)?.intern();
                source = Value::Module(import_file(&mut vm.engine, path, source_span)?);
                replaced_source = true;
            }
            v => {
                bail!(
...
impl Eval for ast::ModuleInclude<'_> {
    ...
    fn eval(self, vm: &mut Vm) -> SourceResult<Self::Output> {
        ...
        let module = match source {
            ...
            v if RootedPath::castable(&v) => {
                let path = v.cast::<RootedPath>().at(span)?.intern();
                import_file(&mut vm.engine, path, span)?
            }
