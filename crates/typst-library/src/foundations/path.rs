...
///
/// - A **relative path** resolves in relation to the parent directory of Typst
///   file where the feature is invoked. While this is the default, a path can
///   also be explicitly specified as being relative by starting it with `./`.
///   ```typ
...
/// a path relative to the file you are currently operating in, but then pass it
...
/// file read or image load), will then behave the same regardless of where in
/// the code they occur.
///
/// Here's an example of how we could have a `main.typ` with a a `data.json`
/// file directly next to it and still let a package we've built read that file.
///
/// ```typ
